...
export default function TabQuickInfo({ quickInfo, onSave }: TabQuickInfoProps) {
  ...
  const handleAdd = () => {
    setRows([...rows, { label: isId ? 'Label Baru' : 'New Label', value: isId ? 'Nilai Baru' : 'New Value' }]);
  };
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-admin-border">
        ...
        >
          ...
          <span>{t('admin.quickInfo.addRow')}</span>
        </button>
      </div>
      ...
      <div className="space-y-3 pt-1">
        {rows.map((row, idx) => (
          ...
          >
            <input
              type="text"
              placeholder={isId ? 'Label (mis. Lokasi)' : 'Label (e.g. Based in)'}
              value={row.label}
            ...
            <input
              type="text"
              placeholder={isId ? 'Nilai (mis. Tegal, Indonesia)' : 'Value (e.g. Tegal, Indonesia)'}
              value={row.value}
      ...
      <div className="pt-4 flex justify-end">
        ...
        >
          <Save size={16} /> {t('admin.quickInfo.save')}
        </button>
      </div>
    </motion.form>
  );
